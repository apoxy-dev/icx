...
func (s *Session) RequestKeys(ctx context.Context, v ICXVersion, addr netip.Addr) (*KeyGrant, error) {
	...
	req := keyRequest{Version: v, RxSPI: myRxSPI, Addr: addr}
	...
	sas, err := s.deriveDirectional(v, myRxSPI, saOffer{Version: grant.Version, RxSPI: grant.RxSPI})
	if err != nil {
		s.bestEffortRelease(grant.VNI)
		return nil, err
	}
...
func (s *Session) handleKeyRequest(stream *quic.Stream, frame []byte, peerPub *ecdsa.PublicKey, granter KeyGranter, granted map[uint32]struct{}, mu *sync.Mutex) {
	...
	sas, err := s.deriveDirectional(req.Version, myRxSPI, saOffer{Version: req.Version, RxSPI: req.RxSPI})
	if err != nil {
	...
	if err := writeFrame(stream, keyGrant{Status: grantOK, Version: req.Version, RxSPI: myRxSPI, VNI: vni}.marshal()); err != nil {
