...
func TestKeyPlaneGrantDerivesMatchingSAs(t *testing.T) {
	...
	grantA, err := sessA.RequestKeys(ctx, AESGCM128, addrA)
	if err != nil {
		t.Fatalf("worker A RequestKeys: %v", err)
	}
	grantB, err := sessB.RequestKeys(ctx, AESGCM128, addrB)
	if err != nil {
...
func TestKeyPlaneConcurrentGrants(t *testing.T) {
	...
	for i := range n {
		...
		go func() {
			...
			grants[i], errs[i] = sess.RequestKeys(ctx, AESGCM128, addr)
		}()
	}
	wg.Wait()
...
func TestKeyPlaneRejectsUnauthorizedPeer(t *testing.T) {
	...
	defer sess.Close()
	if _, err := sess.RequestKeys(ctx, AESGCM128, netip.MustParseAddr("fd61::1")); err == nil {
...
func TestKeyPlaneGranterRejection(t *testing.T) {
	...
	if _, err := sess.RequestKeys(ctx, AESGCM128, badAddr); !errors.Is(err, ErrGrantRejected) {
		t.Fatalf("rejected addr: %v, want ErrGrantRejected", err)
	}
	if _, err := sess.RequestKeys(ctx, AESGCM128, fullAddr); !errors.Is(err, ErrVNIExhausted) {
		t.Fatalf("exhausted space: %v, want ErrVNIExhausted", err)
	}
	// The session must remain usable after rejections.
	if _, err := sess.RequestKeys(ctx, AESGCM128, netip.MustParseAddr("fd61::0:0c")); err != nil {
...
func TestKeyPlaneReleaseAndSessionIsolation(t *testing.T) {
	...
	sessA := dial(workerA)
	sessB := dial(workerB)
	grantA, err := sessA.RequestKeys(ctx, AESGCM128, netip.MustParseAddr("fd61::a"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestKeyPlaneSessionTeardownReleasesGrants(t *testing.T) {
	...
	sess := dial(worker)
	g1, err := sess.RequestKeys(ctx, AESGCM128, netip.MustParseAddr("fd61::1"))
	if err != nil {
		t.Fatal(err)
	}
	g2, err := sess.RequestKeys(ctx, AESGCM128, netip.MustParseAddr("fd61::2"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestKeyPlaneWireFormats(t *testing.T) {
	cases := []struct {
		...
		{
			name:    "key request",
			marshal: keyRequest{Version: AESGCM128, RxSPI: 0x40000007, Addr: netip.MustParseAddr("fd61::9")}.marshal,
			parse: func(b []byte) error {
				got, err := parseKeyRequest(b)
				if err != nil {
					return err
				}
				want := keyRequest{Version: AESGCM128, RxSPI: 0x40000007, Addr: netip.MustParseAddr("fd61::9")}
				if got != want {
		...
		},
		{
			...
			marshal: keyRequest{Version: AESGCM128, RxSPI: 0x40000003, Addr: netip.MustParseAddr("10.0.0.7")}.marshal,
			parse: func(b []byte) error {
				got, err := parseKeyRequest(b)
				if err != nil {
					return err
		...
		{
			name:    "key grant",
			marshal: keyGrant{Status: grantOK, Version: AESGCM128, RxSPI: 0x1234, VNI: 0xabcdef}.marshal,
			parse: func(b []byte) error {
				got, err := parseKeyGrant(b)
				if err != nil {
					return err
				}
				want := keyGrant{Status: grantOK, Version: AESGCM128, RxSPI: 0x1234, VNI: 0xabcdef}
				if got != want {
					return fmt.Errorf("round-trip mismatch: %+v != %+v", got, want)
				}
				return nil
...
func TestDeriveDirectionalRejectsPeerRoleCollision(t *testing.T) {
	...
	if _, err := resp.deriveDirectional(AESGCM128, myRxSPI, saOffer{Version: AESGCM128, RxSPI: spoofed}); err == nil {
	...
	if _, err := resp.deriveDirectional(AESGCM128, myRxSPI, saOffer{Version: AESGCM128, RxSPI: badIdx}); err == nil {
	...
	if _, err := resp.deriveDirectional(AESGCM128, myRxSPI, saOffer{Version: AESGCM128, RxSPI: legit}); err != nil {
...
func TestKeyPlaneManySequentialGrants(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		g, err := sess.RequestKeys(ctx, AESGCM128, addr)
		if err != nil {
...
func TestKeyPlaneReleaseErrorNoDoubleRelease(t *testing.T) {
	...
	g, err := sess.RequestKeys(ctx, AESGCM128, netip.MustParseAddr("fd61::1"))
	if err != nil {
		t.Fatal(err)
	}
	granter.mu.Lock()
...
func TestKeyPlaneRejectsOutOfRangeGranterVNI(t *testing.T) {
	...
	if _, err := sess.RequestKeys(ctx, AESGCM128, netip.MustParseAddr("fd61::1")); !errors.Is(err, ErrGrantRejected) {
