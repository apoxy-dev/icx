...
// RouteLookupForTest resolves (src, dst) through the data-path routing snapshot
// exactly as VirtToPhy does (lock-free Load, LPM on Dst, then LPM on Src) and
// returns the matched VNI. It lets the routing-trie tests assert which network a
// packet resolves to without driving a full encap, and exercises the same
// lock-free path the benchmarks measure.
func (h *Handler) RouteLookupForTest(src, dst netip.Addr) (uint, bool) {
	rt := h.routes.Load()
	value := rt.byDst.Find(dst)
	if value == nil {
		return 0, false
	}
	srcValue := value.(*roDstEntry).srcTrie.Find(src)
	if srcValue == nil {
		return 0, false
	}
...
// the management index, for asserting that emptied entries are reclaimed (APO-654)
